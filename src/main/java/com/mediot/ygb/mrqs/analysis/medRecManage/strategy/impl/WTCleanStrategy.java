...
public class WTCleanStrategy implements DataCleanStrategy {

    private DataCleanRequest dataAnalysisRequest;

    public WTCleanStrategy(DataCleanRequest dataAnalysisRequest){
            ...
            synchronized (lock){
                for(int i=0;i<fileAnalysisDto.getCellNameList().size();i++){
                    for(int j=0;j<fileAnalysisDto.getTDatacleanStandardList().size();j++){
                        ...
                        if(fileAnalysisDto.getStandardCode().equals(StandardCodeEnum.getValue(StandardCodeEnum.WT))){
                            try {
    ...
    public void validateMapOfData(Map<String,Object> resultMap, DataCleanRequest dataAnalysisRequest){
        ...
        try {
            ...
            resultMap.forEach((k,v)->{
                ...
                TDatacleanStandard t=null;
                if(!v.equals(" ")&&!v.equals("")){
                    t=((TDatacleanStandard)v);
                    if(!StringUtils.isBlank(t.getTempCellVal())) {
                        if (t.getDataType().toUpperCase().contains("VAR")) {//字符验证
                            //长度
                            validateDataLength(k, t, errorMsg);
                        } else if (t.getDataType().toUpperCase().contains("NUM")) {
